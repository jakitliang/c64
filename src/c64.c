// Tiny, standalone launcher for c64
// * Sets $C64_VERSION to the release version (-DVERSION)
// * Sets $C64_HOME to the install location
// * Sets $C64_VIM_HOME to the Vim component location
// * Maybe sets $HOME according to c64.ini
// * Starts a BusyBox login shell
//
...
{
    MessageBoxW(0, msg, u"c64 launcher", 0x10);
    ExitProcess(2);
}
...
// Read entries from c64.ini. Expands environment variables, and if "home"
// is relative, converts it to an absolute path. Before the call, the working
// directory must be the location of c64.exe.
static config loadconfig(arena *perm, arena scratch)
{
    config conf = newconfig();

    s8 ini = loadfile(u"c64.ini", &scratch);
...
static u32 c64(void)
{
    ...
    SetEnvironmentVariableW(u"C64_HOME", path.buf);  // ignore errors

    path = moduledir;
    buf16cat(&path, U(u"\\opt\\vim"));
    buf16c16(&path, 0);  // null terminator
    SetEnvironmentVariableW(u"C64_VIM_HOME", path.buf);  // ignore errors

    #ifdef VERSION
    #define LSTR(s) XSTR(s)
    #define XSTR(s) u ## # s
    SetEnvironmentVariableW(u"C64_VERSION", LSTR(VERSION));  // ignore errors
    #endif

    // Maybe set HOME from c64.ini.
    config conf = newconfig();
    path = moduledir;
    buf16c16(&path, 0);
    ...
    // Preserve the inherited PATH, then install only c64's stable base layer.
    // Toolchains are activated explicitly through use.cmd.
    buf16 basepath = newbuf16(perm, MAX_ENVVAR);
    buf16getenv(&basepath, u"PATH", scratch);
    buf16c16(&basepath, 0);
    SetEnvironmentVariableW(u"C64_BASE_PATH", basepath.buf);  // ignore errors
    SetEnvironmentVariableW(u"C64_TOOLCHAIN", 0);  // ignore errors
    SetEnvironmentVariableW(u"C64_TARGET", 0);  // ignore errors
    SetEnvironmentVariableW(u"C64_GCC_HOME", 0);  // ignore errors
    SetEnvironmentVariableW(u"C64_CCACHE", 0);  // ignore errors

    path = moduledir;
    buf16cat(&path, U(u"\\bin;"));
    buf16cat(&path, (s16){basepath.buf, basepath.len - 1});
    buf16c16(&path, 0);  // null terminator
