...
在 64 位 Windows 上，一个 c64 安装目录可同时提供两种目标。下载版本相同的
`c64-i686.zip` 发行包，然后仅将其中的 `c64/mingw32/` 目录解压并合并到现有
`c64.zip` 安装目录的根目录。合并后结构如下：
...
必须使用相同版本的发行包，不要混合不同 c64 版本的工具链目录。也不要将完整
i686 压缩包覆盖解压到 x64 安装目录中；只合并 `mingw32/`。共享的启动器、基础工具
和配置应保留 `c64.zip` 中的版本。

合并后的 i686 工具链仍保留 i686 发行包的 Windows XP API 基线和 Pentium 4 CPU
基线，因此可在 64 位 Windows 主机上构建面向 XP 的程序；32 位编译器程序会通过
WoW64 运行。
...
使用 `use status` 确认 `C64_TARGET` 为 `i686-w64-mingw32`。标准 64 位 Windows
可通过 WoW64 运行 32 位编译器程序。这样，一个便携式 c64 目录即可同时构建 64 位
和 32 位应用程序。
